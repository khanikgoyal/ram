...
const registerUser = asyncHandler(async(req,res)=>{
    ...
    const coverImageLocalPath= (req.files?.coverImage||null)?.path
...
const refreshAccessToken = asyncHandler(async (req, res )=>{
    ...
    try {
        ...
        .cookie("accessToken", accessToken, options)
        .cookie("refreshTokenToken", newrefreshToken, options)
        .json(
            new ApiResponse(
                200,
...
const changeCurrentPassword= asyncHandler(async(req, res)=>{
    ...
    const isPassworsCorrect=await user.isPasswordCorrect(oldPassword)
